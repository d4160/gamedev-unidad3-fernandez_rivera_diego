...
namespace Bitgem.VFX.StylisedWater
{
    ...
    public class WaterVolumeBase : MonoBehaviour
    {
        ...
        {
            var localPos = transform.InverseTransformPoint(_position);
            var x = Mathf.FloorToInt((localPos.x + 0.5f) / TileSize);
            var z = Mathf.FloorToInt((localPos.z + 0.5f) / TileSize);
            ...
            for (var y = MAX_TILES_Y - 1; y >= 0; y--)
            {
                if (tiles[x, y, z])
                {
                    return transform.position.y + (y + 1) * TileSize;
                }
            }
